...
//			printf("name: %s, theme:%d\n",name,g_device->options&T_THEME);
	...
	//ESP_LOGV(TAG,"Heap size: %d",esp_get_free_heap_size());
	gpart = PART;
	if(f != NULL)
	{
		length = f->size;
			...
			if (getSParameterFromResponse(id,6,"idgp=", data, data_size) )
			{
				if ((atoi(id) >=0) && (atoi(id) < 255))
				{
					...
					if (buf == NULL)
					{
						...
						sprintf(buf, strsGSTAT, json_length, si->name, si->domain, si->file,si->port,si->ovol);
...
//printf("data:%s\n",data);
		...
		update_firmware((char*)"KaRadio");  // start the OTA
