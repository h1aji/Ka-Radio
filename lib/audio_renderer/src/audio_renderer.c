...
// har-in-air correction	
	...
	if (renderer_instance->bit_depth == I2S_BITS_PER_SAMPLE_24BIT) outBufBytes <<= 1;
...
//	ESP_LOGI(TAG, "render_spdif_samples len: %d, bytes_cnt: %d",buf_len,bytes_cnt);
	...
	uint32_t *spdif_buffer = heap_caps_malloc(bytes_cnt, MALLOC_CAP_8BIT);
...
// output_mode *****************************
    if(config->output_mode == I2S_MERUS) {
        config->bit_depth = I2S_BITS_PER_SAMPLE_24BIT;
    ...
    {
		config->bit_depth = I2S_BITS_PER_SAMPLE_16BIT;
        mode = mode | I2S_MODE_MASTER;
		comm_fmt = I2S_COMM_FORMAT_I2S_MSB;
	}
    else if(config->output_mode == PDM)
    {
        mode = mode | I2S_MODE_SLAVE;

		comm_fmt = I2S_COMM_FORMAT_I2S | I2S_COMM_FORMAT_I2S_LSB;
    }
    else if(config->output_mode == SPDIF)
    {
//		comm_fmt = I2S_COMM_FORMAT_STAND_MSB;
		bit_depth = I2S_BITS_PER_SAMPLE_24BIT;
